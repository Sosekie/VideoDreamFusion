...
def maybe_fallback_attn_mode(attn_mode):
    ...
    # Allow forcing torch attention mode via environment variable to avoid flash attention issues
    force_torch_attn = os.environ.get('FORCE_TORCH_ATTN', '').lower() in ('1', 'true', 'yes')
    if force_torch_attn and attn_mode in ('flash', 'flash2', 'flash3'):
        warnings.warn("Forcing torch attention mode due to FORCE_TORCH_ATTN environment variable.")
        return 'torch'

    # Allow forcing flash2 instead of flash3 via environment variable
    force_flash2 = os.environ.get('FORCE_FLASH2', '').lower() in ('1', 'true', 'yes')
    ...
    if attn_mode == 'flash':
        if is_flash3_available() and not force_flash2:
            attn_mode = 'flash3'
        ...
        if force_flash2 and is_flash2_available():
            attn_mode = 'flash2'
            warnings.warn("Forcing flash2 instead of flash3 due to FORCE_FLASH2 environment variable.")
        elif not is_flash3_available():
            attn_mode = 'torch'
