#!/bin/bash
# ==========================================
# H100 交互式 GPU 申请脚本 (UBELIX 2025 Update)
# ==========================================
  ...
  srun --partition=${PARTITION} --qos=${QOS} --account=${ACCOUNT} --nodes=1 --gpus=${GPU_TYPE}:1 --cpus-per-task=4 --mem=${MEM_SIZE} --time=${TIMEFMT} --pty bash -c '
    send_notice() {
      ...
      if command -v mailx >/dev/null 2>&1; then
        ...
        printf "%s" "$body" | mail -s "GPU H100 申请成功" ${MAIL_ADDR} || echo "⚠️ 邮件可能未发送成功 (mail)"
      else
        echo "⚠️ 系统未安装 mail/mailx，跳过发信"
      fi
    }

    echo "🎉 GPU H100 资源申请成功！"
    echo "📧 正在发送邮件通知..."

    send_notice "🎉 GPU H100申请成功 - ${QOS_TAG}" "GPU H100 资源申请成功！\n\n详细信息：\n• 申请时间：$(date "+%Y-%m-%d %H:%M:%S")\n• 申请时长：${HOURS_STR}小时\n• QoS类型：${QOS_TAG}\n• 账户：${ACCOUNT}\n• 分区：${PARTITION}\n• 分配节点：$(hostname)\n• 用户：$USER\n• GPU类型：H100\n${EXTRA_NOTE}\n\n你可以开始使用GPU资源进行训练/调试了！\n记得在使用完毕后及时释放资源。\n\n祝你顺利！🚀"
